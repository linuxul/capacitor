...
- A permission result goes to the call that is waiting on that `@PermissionCallback`. When two permission requests of one plugin overlapped (for example `requestPermissions` while `getPhoto` waits on the camera prompt), the first result to come back answered the plugin's earliest waiting call, even one waiting on the other callback.
...
- Local server `Range` requests: a partial response (206) ends at the last byte of the range; it used to run to the end of the file. A range that starts past the end, or `bytes=-0`, is answered with 416 and `Content-Range: bytes */<size>`, as on iOS. A malformed Range header no longer crashes the app, but the Android WebView itself fails such a request (`net::ERR_REQUEST_RANGE_NOT_SATISFIABLE`) before the local server's response is read, where iOS serves the whole file. Asset, file and content streams are closed after the WebView reads them instead of leaking a file descriptor each.
