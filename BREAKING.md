...
- `DisableDeploy` and `KeepRunning` came from Cordova's `config.xml` preferences and are gone. The runtime behaves as it did with their defaults.
...
- Removed deprecated API: `CAPBridge`, `CAPFileManager`, `JSDate`, `JSResultBody`, the `CAPNotifications` enum; `getWebView`, `isSimulator`, `isDevMode`, `getStatusBarVisible`, `getStatusBarStyle`, `getUserInterfaceStyle`, `getLocalUrl`, `getSavedCall`, `releaseCall(callbackId:)`, `presentVC`, `dismissVC`, `modulePrint` and the status bar setters on the bridge; `isSaved`, `save()`, `hasOption` and the initializer without a method name on `CAPPluginCall`; `getConfigValue`, `supportsPopover` and `init(bridge:pluginId:pluginName:)` on `CAPPlugin`; `getAny` on `JSValueContainer`; `legacyConfig`, `getPluginConfigValue`, `getValue` and `getString` on the instance configuration.
...
- A `@PluginMethod` function must be public and take exactly one `PluginCall`; anything else is rejected with an `InvalidPluginException` when the plugin is registered. In Kotlin that rules out `internal`, `suspend` and value class parameters, all of which change the JVM method name or signature. `@PermissionCallback` and `@ActivityCallback` functions may be private but not `internal`.
